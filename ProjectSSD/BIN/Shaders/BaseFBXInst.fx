...
VTX_OUTPUT VS_Main(VTX_INPUT _Input)
{
    ...
    OutData.vPos = mul(float4(_Input.vPos, 1.f), _Input.matWVP);
    OutData.vTexCoord = _Input.vTexCoord;

    OutData.vNormal = normalize(mul(_Input.vNormal, (float3x3) _Input.matWV));
    OutData.vBiNormal = normalize(mul(_Input.vBiNormal, (float3x3) _Input.matWV));
    OutData.vTangent = normalize(mul(_Input.vTangent, (float3x3) _Input.matWV));
    OutData.vViewPos = mul(float4(_Input.vPos, 1.f), _Input.matWV).xyz;
...
PIX_OUTPUT PS_Main(VTX_OUTPUT _Input)
{
    ...
    OutData.Depth = float4(_Input.vViewPos.z, 0.f, _Input.vViewPos.z / Far, 1.f);
