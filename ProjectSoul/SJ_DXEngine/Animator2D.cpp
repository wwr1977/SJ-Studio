...
{
	m_SpriteRenderer = ACTOR->CreateCom<CSprite2DRenderer>(RENDATA{ _InitData.RenderLayer});
...
{
	m_SpriteRenderer = ACTOR->CreateCom<CSprite2DRenderer>(_InitData.SpriteRenData);
...
void CAnimator2D::SetDepthState(const wchar_t* _DepthKey) 
{
	if (nullptr == m_SpriteRenderer)
		return;

	m_SpriteRenderer->SetDepthState(_DepthKey);
}
void CAnimator2D::SetBlendState(const wchar_t* _BlendKey) 
{
	if (nullptr == m_SpriteRenderer)
		return;

	m_SpriteRenderer->SetBlendState(_BlendKey);
}
...
void CAnimator2D::SetGrayScale(const bool& _Gray)
{
	m_SpriteRenderer->SetGrayScale(_Gray);
}
void CAnimator2D::YSortingOn()
{
	if (nullptr == m_SpriteRenderer)
		return;

	m_SpriteRenderer->YSortingOn();
}
void CAnimator2D::YSortingOff()
{
	if (nullptr == m_SpriteRenderer)
		return;

	m_SpriteRenderer->YSortingOff();
}
const RENDER_SORTING CAnimator2D::GetRenSortingType()
{
	if (nullptr == m_SpriteRenderer)
		return MAXRENDERSORTING;

	return m_SpriteRenderer->GetRenSortingType();
}
