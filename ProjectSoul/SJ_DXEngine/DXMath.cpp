...
const float DXMath::Epsilon = 0.0000001f;
...
const Vec2 DXMath::Vec2Slerp(const Vec2& _Start, const Vec2& _End, const float& _Weight)
{
	...
	if (std::isnan(SlerpVec.x) || std::isnan(SlerpVec.y))
		TASSERT(true);
...
const _tagVEC2 DXMath::EpsilonCheck(const _tagVEC2& _Vec2)
{
	Vec2 Result =_Vec2;
...
const _tagVEC3 DXMath::EpsilonCheck(const _tagVEC3& _Vec3)
{
	Vec3 Result = _Vec3;
	Result.x = ExceptSmallNumber(Result.x);
	Result.y = ExceptSmallNumber(Result.y);
